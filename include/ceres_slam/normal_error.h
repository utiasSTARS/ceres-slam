...
class NormalErrorAutomatic {
   public:
    ...
    NormalErrorAutomatic(const Vector &obs_normal_c,
        ...
        : obs_normal_c_(obs_normal_c), stiffness_(stiffness) {}

    //! Templated evaluator operator for use with ceres::Jet
    template <typename T>
    bool operator()(const T *const T_c_g_ceres, const T *const normal_g_ceres,
        ...
        residuals = stiffness_.cast<T>() * (normal_c - obs_normal_c_.cast<T>());
