...
class SO3GroupBase {
   public:
    ...
    //! Transformed coordinate Jacobian matrix w.r.t. transformation (3 x 9)
    typedef Eigen::Matrix<Scalar, dim, dim * dim, Eigen::RowMajor>
        TransformJacobian;

    //! Return a reference to the underlying matrix
    inline StorageType& matrix() {
        return static_cast<Derived*>(this)->matrix();
    }
    //! Return a const reference to the underlying matrix
    inline const StorageType& matrix() const {
        return static_cast<const Derived*>(this)->matrix();
    }
    //! Return a pointer to the raw matrix data
    inline Scalar* data() { return this->matrix().data(); }
    //! Return a const pointer to the raw matrix data
    inline const Scalar* data() const { return this->matrix().data(); }
    ...
    inline SO3GroupBase<Derived>& operator=(
    ...
    inline void normalize() {
        ...
        middle(2, 2) =
            svd.matrixV().determinant() * svd.matrixU().determinant();
    ...
    /*!
        Transform a 3-vector and, if requested, compute the
        Jacobian of the transformed vector w.r.t. the matrix elements
    */
    inline const Vector transform(
        ...
        if (jacobian_ptr != nullptr) {
            ...
            jacobian.block(0, 0, 1, 3) = v.transpose();
            jacobian.block(1, 3, 1, 3) = v.transpose();
            jacobian.block(2, 6, 1, 3) = v.transpose();
        }
    ...
    inline static const SO3Group<Scalar> Identity() {
    ...
    */
    inline static const TransformationMatrix wedge(const TangentVector& phi) {
        ...
        Phi << static_cast<Scalar>(0), -phi(2), phi(1), phi(2),
            static_cast<Scalar>(0), -phi(0), -phi(1), phi(0),
    ...
    */
    inline static const TangentVector vee(const TransformationMatrix& Phi) {
        TangentVector phi;
        phi << Phi(2, 1) - Phi(1, 2), Phi(0, 2) - Phi(2, 0),
            Phi(1, 0) - Phi(0, 1);
    ...
    */
    inline static const SO3Group<Scalar> exp(const TangentVector& phi) {
        ...
        TransformationMatrix mat =
            cp * TransformationMatrix::Identity() +
...
class SO3Group : public SO3GroupBase<SO3Group<_Scalar, _Options> > {
   ...
   public:
    //! Scalar type
    typedef
        typename Eigen::internal::traits<SO3Group<_Scalar, _Options> >::Scalar
            Scalar;
    //! Internal storage type
    ...
    SO3Group(const TransformationMatrix& matrix) : matrix_(matrix) {}
