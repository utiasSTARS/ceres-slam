...
int main() {
    ...
    Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> P_left(
        left_camera_info->P.data());
    P_left << fu, 0, cu, 0, 0, fv, cv, 0, 0, 0, 1, 0;

    Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor>> P_right(
