...
public:
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& left_image,
        ...
        if(prev_left_keypoints_.size() == 0) {
            ...
            matcher.match(prev_left_descriptors_,
                          curr_left_descriptors,
                          vo_matches);
            ...
            std::vector<ceres_slam::StereoCamera::Observation>
                prev_obs, curr_obs;
            std::vector<ceres_slam::StereoCamera::Point>
                prev_pts, curr_pts;
            ...
            std::vector<ceres_slam::StereoCamera::Observation>
                prev_inlier_obs, curr_inlier_obs;
            std::vector<ceres_slam::StereoCamera::Point>
                prev_inlier_pts, curr_inlier_pts;
            ...
            double curr_rot[3];
            Eigen::Matrix<double, 3, 3, Eigen::RowMajor> C_curr_prev = T_curr_prev.linear();
            ceres::RotationMatrixToAngleAxis<double>(C_curr_prev.data(), curr_rot);
            ...
            Eigen::Vector3d t_curr_prev = -C_curr_prev.transpose() * T_curr_prev.translation();
            double curr_trans[3];
            ...
            for(unsigned int i = 0; i < inlier_idx.size(); ++i) {
                ...
                problem.AddResidualBlock(prev_stereo_cost, NULL,
                                         prev_trans, prev_rot,
                                         prev_inlier_pts[i].data());
                ...
                problem.AddResidualBlock(curr_stereo_cost, NULL,
                                         curr_trans, curr_rot,
                                         curr_inlier_pts[i].data());
            }

            Solve(solver_options, &problem, &summary);

            // Now put the solution back in the format we want
            ceres::AngleAxisToRotationMatrix<double>(curr_rot, C_curr_prev.data());
    ...
    void publishVOTracks(cv_bridge::CvImagePtr image_ptr,
        ...
        sensor_msgs::ImagePtr msg = cv_bridge::CvImage(
            std_msgs::Header(), "mono8", image_ptr->image).toImageMsg();
    }
